...
chaincodeList() {
  if [ "$#" -ne 2 ]; then
    ...
    peerChaincodeList "cli.curator.local" "peer0.curator.local:7041" "$2" # $2 is channel name

  elif
    [ "$1" = "peer0.partner1.local" ]
  then

    peerChaincodeList "cli.partner1.local" "peer0.partner1.local:7061" "$2" # $2 is channel name
